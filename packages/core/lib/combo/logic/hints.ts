...
export class LogicPassHints {
  ...
  private makeHints(world: number): WorldHints {
    /* Compute foolish regions */
    const foolish = this.foolishRegions(world);

    /* Get static hinted specific items */
  ...
  run() {
    ...
    const hints: Hints = [];
    for (let world = 0; world < this.state.settings.players; ++world) {
      hints.push(this.makeHints(world));
    }
